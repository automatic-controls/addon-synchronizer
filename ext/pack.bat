...
:packAddon
  ...
  for /f "usebackq tokens=1,2,* delims==" %%i in ("%trackingRecord%") do (
    set "p=%%k"
    if "!p!" EQU "!p:src\aces\webctrl\sync\database=!" (
      robocopy /E "%trackingClasses%\%%i" "%classes%" >nul 2>nul
    )
  )
  robocopy /E "%src%" "%classes%" /XF "*.java" /XD "%src%\aces\webctrl\sync\database" >nul 2>nul
...
:packDatabase
  ...
  for /f "usebackq tokens=1,2,* delims==" %%i in ("%trackingRecord%") do (
    set "p=%%k"
    if "!p!" EQU "!p:src\aces\webctrl\sync\addon=!" (
      robocopy /E "%trackingClasses%\%%i" "%databaseRoot%" >nul 2>nul
    )
  )
  robocopy /E "%src%" "%databaseRoot%" /XF "*.java" /XD "%src%\aces\webctrl\sync\addon" >nul 2>nul
