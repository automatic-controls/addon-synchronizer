@echo off
title Add-On Synchronizer Installation Manager
...
:generateServiceXML
  (
    echo ^<service^>
    echo   ^<id^>addonSynchronizerWebCTRL^</id^>
    echo   ^<name^>Addon Synchronizer^</name^>
    echo   ^<description^>This service manages a database which is intended to interface with corresponding WebCTRL add-ons.^</description^>
    echo   ^<executable^>%%BASE%%\jre\bin\java^</executable^>
    echo   ^<arguments^>-cp "%%BASE%%\*" aces.webctrl.sync.database.Main^</arguments^>
